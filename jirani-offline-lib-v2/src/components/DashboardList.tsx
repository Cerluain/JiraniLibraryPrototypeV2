...
export default function DashboardList({ loadViewPage }: Props) {
    ...
    const list_of_media_cards =
        jsonFiles && jsonFiles.length > 0 ?
            jsonFiles.filter(jsonFile => jsonFile.filename).map((json_file) => {

                console.log("Fetching internal filename: ", json_file.filename);
                return <MediaCard clickEvent={loadViewPage} filename={json_file.filename} book_title={json_file.title} author={json_file.author} date={json_file.timestamp} key={json_file.timestamp} />
            }
            )
            : <h1>No Files</h1>;
